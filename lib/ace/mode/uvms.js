...
define(function(require, exports, module) {
    ...
    var UvmsHighlightRules = require("./uvms_highlight_rules").UvmsHighlightRules;
    var UvmsFoldMode = require("./folding/uvms").FoldMode;
    var Range = require("../range").Range;
    var WorkerClient = require("../worker/worker_client").WorkerClient;
    
    var Mode = function() {
        this.HighlightRules = UvmsHighlightRules;
        
        this.foldingRules = new UvmsFoldMode();
    ...
    (function() {
        ...
        function getNetIndentLevel(tokens) {
            ...
            for (var i = 0; i < tokens.length; i++) {
                var token = tokens[i];
                if(token.value === '.' && tokens.length>i+1) {
                    token.value = token.value + tokens[i+1].value;
                    token.type = 'keyword';
                }
        ...
        this.createWorker = function(session) {
            var worker = new WorkerClient(["ace"], "ace/mode/uvms_worker", "Worker");
